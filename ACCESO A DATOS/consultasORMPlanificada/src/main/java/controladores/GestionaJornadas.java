...
logger.info("-------------------------PARTICIPANTES--------------------");
		List<Participante> participantes = participanteDao.obtenerparticipantenombreascdentemente();
		for (Participante p : participantes) {
			logger.info(p.getNombre()," ",p.getApellidos()," ",p.getEmail());
		}
		logger.info("-------------------------EVENTOS--------------------");

		List<Object[]> eventosmas91min = eventoDao.obtenereventosqueduranmasde91min();
		for (Object e : eventosmas91min) {
			logger.info(e);
		}
		List<Evento> eventosenubicacion=eventoDao.obtenereventosporubicacion(u2.getNombre());
		
		for (Evento evento : eventosenubicacion) {
		logger.info(evento);	
		}
