...
class IndicadorIncidencias(QWidget):
    ...
    def cambiarcolor(self,cantidad):
        self.contador=cantidad
        if self.contador > 1 and self.contador < 3:
            self.color=(QColor("Green"))
            self._texto="OK"
        elif self.contador > 4 and self.contador < 7:
...
class EtiquetaContadorIncidencias(QLabel):
    ...
    def actualizar_contador(self, cantidad):
        ...
        if cantidad > 1 and cantidad < 3:
            paleta.setColor(QPalette.Window, QColor("Green"))
...
class Reset(QPushButton):
    ...
    def reiniciar(self):
        self.rojo= QColor("White")
        self.text
        self.reset.emit(self.emite)
...
class VentanaPrincipal(QMainWindow):
    def __init__(self):
        ...
        self.boton.incidencias.connect(self.indicador.cambiarcolor)
        self.botonreiniciar.clicked.connect(self.reiniciarprincipal)
        self.botonreiniciar.reset.connect(self.etiqueta.actualizar_contador)
    ...
    def reiniciarprincipal(self):
        self.indicador.reiniciar()
